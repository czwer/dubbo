...
public class AwaitingNonWebApplicationListener implements SmartApplicationListener {
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (event instanceof ApplicationReadyEvent) {
            logger.info("自定义日志---【监听事件】ApplicationEvent(ApplicationReadyEvent)，timestamp：" + event.getTimestamp());
            onApplicationReadyEvent((ApplicationReadyEvent) event);
        }

        if (event instanceof ApplicationFailedEvent) {
            logger.info("自定义日志---【监听事件】ApplicationEvent(ApplicationFailedEvent)，timestamp：" + event.getTimestamp());
    ...
    protected void onApplicationReadyEvent(ApplicationReadyEvent event) {
        logger.info("自定义日志---【监听事件】ApplicationReadyEvent，timestamp：" + event.getTimestamp());
