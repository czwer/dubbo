...
public class DubboDeployApplicationListener
    ...
    private void publishApplicationEvent(DeployState state) {
        logger.info("自定义日志---【发布事件】DubboApplicationStateEvent");
    ...
    public void onApplicationEvent(ApplicationContextEvent event) {
        if (nullSafeEquals(applicationContext, event.getSource())) {
            if (event instanceof ContextRefreshedEvent) {
                logger.info("自定义日志---【监听事件】ApplicationContextEvent(ContextRefreshedEvent)，timestamp："
                        + event.getTimestamp());
                onContextRefreshedEvent((ContextRefreshedEvent) event);
            } else if (event instanceof ContextClosedEvent) {
                logger.info(
                        "自定义日志---【监听事件】ApplicationContextEvent(ContextClosedEvent)，timestamp：" + event.getTimestamp());
